...
export default function ManagerDashboard() {
  ...
  async function load() {
    setLoading(true);
    const data = await api.get<FundRequest[]>("/fund-requests/pending-approval");
    setRequests(data);
    setLoading(false);
  }
  ...
  async function decide(id: string, approve: boolean) {
    setBusyId(id);
    try {
      await api.patch(`/fund-requests/${id}/manager-decision`, { approve, comment: comments[id] });
      await load();
    } finally {
      setBusyId(null);
    }
      ...
      {loading ? (
        ...
        requests.map((r: any) => (
          <div className="card" key={r.id}>
            <p>
              <strong>{r.requester?.name}</strong> ({r.requester?.email}) requests{" "}
              <strong>
                {r.currency} {r.amount}
              </strong>
            </p>
